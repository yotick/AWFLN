...
def qindex(img1, img2, block_size=8):
    ...
    if img1.ndim == 2:
        ...
        qindexs = [_qindex(img1[i, ...], img2[i, ...], block_size) for i in range(img1.shape[0])]
...
#####  testing ##################
# imgf = 'D:\\temppic\\fsplout\\140.tif'
# imglr = 'D:\\temppic\\plei\\ms\\140.tif'
# imglru = 'D:\\temppic\\plei\\lr4\\140.tif'
# imgpan = 'D:\\temppic\\plei\\pan\\140.tif'
#
# imgf=Image.open(imgf)
# imglr=Image.open(imglr)
# imglru=Image.open(imglru)
# imgpan=Image.open(imgpan)
# imgf=np.array(imgf)
# imglr=np.array(imglr)
# imglru=np.array(imglru)
# imgpan=np.array(imgpan)
#
# x1=np.expand_dims(imgpan, 2)
# #
# # x1 = np.random(800,800,1)
# # x1[:,:,1]=imgpan
# #
# c=qnr(img_fake=imgf,img_lm=imglru,pan=x1,satellite='QuickBird', scale=1, block_size=32, p=1, q=1, alpha=1, beta=1)
# # qnr(img_fake, img_lm, pan, satellite='QuickBird', scale=4, block_size=32, p=1, q=1, alpha=1, beta=1):
#
# print(c)
